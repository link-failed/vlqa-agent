...
def get_relevant_table_info(task_id: str) -> str:
    # Read close tables CSV
    close_tables_path = "data/encode/close_tables.csv"
    close_tables_df = pd.read_csv(close_tables_path)
    
    # Get relevant table names for task_id 
    mask = close_tables_df['task_id'].astype(str) == str(task_id)
    table_row = close_tables_df[mask]
    if table_row.empty:
        return "row empty"
    table_names = table_row['relevant_table_info'].iloc[0].split(", ")
    # print(f"Table names: {table_names}")
    
    # Read table info mapping
    mapping_path = "data/encode/table_info_encode_mapping.json"
    with open(mapping_path, 'r') as f:
        table_mappings = json.load(f)
    
    # Get text content for relevant tables
    relevant_texts = []
    for table_name in table_names:
        for mapping in table_mappings:
            if mapping['file'] == table_name:
                table_info = f"=== {table_name} info ===\n" + mapping['text']
                relevant_texts.append(table_info)
                break
...
def run_single_task(
    ...
    if is_reasoning_llm(model_id):
        relevant_table_info = get_relevant_table_info(task["task_id"])
        prompt = reasoning_llm_task_prompt.format(
            question=task["question"],
            guidelines=task["guidelines"],
            referred_examples="",
            relevant_table_info=relevant_table_info
        )
...
def main():
    ...
    runs_dir = Path().resolve() / "encode_res/runs"
