...
def get_field_dependency_adjacency(input_fields: List[str], output_fields: List[str] = None) -> Dict[str, List[str]]:
    """
    Standalone function to get adjacency dictionary for field dependencies.
    This function can be imported by other modules like run2-subgraph.py.
    
    Args:
        input_fields: List of input node names (e.g., ['payments.merchant', 'payments.day_of_year'])
        output_fields: List of output node names (can be empty)
    ...
    Example:
        >>> adj_dict = get_field_dependency_adjacency(['payments.merchant'], ['fees.ID'])
        >>> print(adj_dict['payments.merchant'])
        ['merchant_data.merchant', 'payments.aci', 'payments.eur_amount', ...]
    """
    ...
    adjacency_dict = field_graph.get_subgraph_adjacency_list(reachable_nodes)
    
    return adjacency_dict


def analyze_field_dependencies(input_fields: List[str], output_fields: List[str] = None, verbose: bool = False) -> Dict[str, Any]:
    """
    Comprehensive field dependency analysis function that can be imported by other modules.
    ...
    Returns:
        ...
        - adjacency_list: Adjacency list of the subgraph
        - subgraph_stats: Statistics about the subgraph
        - input_fields: Original input fields
        - output_fields: Original output fields
    """
    # Create a graph instance
    field_graph = FieldDependencyGraph()
    
    # Perform comprehensive analysis
    analysis_result = field_graph.analyze_reachable_subgraph(input_fields, output_fields)
    
    if verbose:
        # Print detailed analysis
        field_graph.print_reachable_analysis(input_fields, output_fields, analysis_result['reachable_nodes'])
