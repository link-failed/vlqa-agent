...
Answer must be just the selected ACI to incentive and the associated cost rounded to 2 decimals in this format: {aci}:{fee}. If a question does not have a relevant or applicable answer for the task, please respond with 'Not Applicable'.
"""
"""
...
def parse_capture_delay(fee_capture_delay):
    ...
    if isinstance(fee_capture_delay, str):
        if fee_capture_delay.startswith("<"):
            ...
            try:
                int(fee_capture_delay)  # Check if it can be parsed into a number
                return [fee_capture_delay]
            except ValueError:
                return [fee_capture_delay]  # Keep as string if not parsable
...
# get merchant information to filter fees and payments
...
for aci in aci_candidates:
    ...
    print(f"ACI: {aci}, Fee: {aci_fee:.2f}")
